...
def resize3(src_img, ratio):
    ...
    for i in range(dst_h):
        for j in range(dst_w):
            ...
            dst_img[i, j, :] = (int((1. - src_y_float)*mkernel) * int((1. - src_x_float)*mkernel) * src_img[src_y_int, src_x_int, :] + \
                               int((1. - src_y_float)*mkernel) * int(src_x_float*mkernel) * src_img[src_y_int, src_x_int + 1, :] + \
                               int(src_y_float*mkernel) * int((1. - src_x_float)*mkernel) * src_img[src_y_int + 1, src_x_int, :] + \
                               int(src_y_float*mkernel) * int(src_x_float*mkernel) * src_img[src_y_int + 1, src_x_int + 1, :]) >> 22
    return dst_img
